...
class RendererBatchAdapter:
    ...
    def adapt(self, sample: dict[str, object]) -> PatchBatch:
        ...
        semantic_embed = np.asarray(contract.get("semantic_embed", [1.0, 0.0, 0.0, 0.3, 0.2, 0.2]), dtype=np.float32)
...
class RendererTrainer:
    ...
    @staticmethod
    def evaluate_model(model: TrainableLocalPatchModel, batches: list[PatchBatch]) -> dict[str, float]:
        if not batches:
            return {"reconstruction_mae": 1.0, "alpha_mae": 1.0, "uncertainty_calibration_mae": 1.0, "contract_validity": 0.0, "fallback_free_happy_path_ratio": 0.0, "face_family_score": 0.0, "torso_family_score": 0.0, "sleeve_family_score": 0.0, "score": 0.0}
    ...
    def train(self, config: TrainingConfig) -> StageResult:
        ...
        for epoch in range(config.epochs):
            ...
            eval_metrics = self.evaluate_model(self.model, val_batches)
