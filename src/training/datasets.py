        ...
        for idx, p in enumerate(rec.get("persons", [])):
            ...
            persons.append(
                PersonFacts(
                    ...
                    expression_source=str(p.get("expression_source", "training_synthetic")),
                    orientation=OrientationState(),
                    orientation_confidence=float(p.get("orientation_confidence", 0.0)),
                    orientation_source=str(p.get("orientation_source", "training_synthetic")),
                    track_id=str(p.get("track_id", f"track_cached_{idx}")),
                    track_confidence=float(p.get("track_confidence", 0.0)),
                    track_source=str(p.get("track_source", "training_synthetic")),
                    garments=list(p.get("garments", [])),
                    person_id=str(p.get("person_id", f"person_cached_{idx}")),
                    bbox_observation_status=str(p.get("bbox_observation_status", "unknown")),
                    mask_evidence_type=str(p.get("mask_evidence_type", "unknown" if not p.get("mask_ref") else "training_synthetic")),
                    track_provenance=str(p.get("track_provenance", p.get("track_source", "training_synthetic"))),
                    identity_observation_status=str(p.get("identity_observation_status", "training_synthetic")),
                    suitable_for_memory_seeding=bool(p.get("suitable_for_memory_seeding", False)),
                )
            )
...
class DynamicsDataset(BaseStageDataset):
    ...
    @classmethod
    def _from_observed_graph_delta_manifest(cls, payload: dict[str, object], manifest_path: str,
                                            strict: bool = False) -> "DynamicsDataset":
        ...
        for idx, rec in enumerate(records):
            try:
                ...
                if ts in {"heuristic_runtime_prediction", "legacy_graph_delta_fallback",
                          "self_generated_runtime_target"}:
                    diagnostics["rejected_runtime_or_heuristic_targets"] = int(
                        diagnostics["rejected_runtime_or_heuristic_targets"]) + 1
                ...
                delta = GraphDelta(
                    ...
                    semantic_reasons=[str(x) for x in td.get("semantic_reasons", [str(
                        (rec.get("transition_context", {}) or {}).get("family", "pose_transition"))])],
                    region_transition_mode={str(k): str(v) for k, v in
                                            (td.get("region_transition_mode", {}) or {}).items()},
                )
                if not (
                        delta.pose_deltas or delta.garment_deltas or delta.visibility_deltas or delta.expression_deltas or delta.interaction_deltas):
                    raise ValueError("empty_target_delta_groups")
                fam = str((rec.get("transition_context", {}) or {}).get("family", "pose_transition"))
                diagnostics["transition_family_counts"][fam] = int(
                    diagnostics["transition_family_counts"].get(fam, 0)) + 1
                for r in rec.get("affected_regions", []) if isinstance(rec.get("affected_regions", []), list) else []:
                    diagnostics["affected_region_counts"][str(r)] = int(
                        diagnostics["affected_region_counts"].get(str(r), 0)) + 1
                out.append(
                    {"graphs": [before, apply_delta(before, delta)], "actions": [ActionStep(type=fam, priority=1)],
                     "deltas": [delta], "source": "manifest_dynamics_graph_delta_supervised",
                     "graph_transition_contract": {"metadata": {"transition_family": fam}}, "target_source": ts,
                     "training_target_quality": tq, "target_training_role": tr})
