...
def main() -> None:
    ...
    if args.stage == "renderer_manifest_from_observed_pairs":
    ...
    elif args.stage == "dynamics_manifest_from_observed_transitions":
        if not args.observed_transitions_path:
            raise ValueError("--observed-transitions-path is required for dynamics_manifest_from_observed_transitions")
        if not args.output_path:
            raise ValueError("--output-path is required for dynamics_manifest_from_observed_transitions")
        result = build_dynamics_manifest_from_observed_transitions(observed_transitions_path=args.observed_transitions_path, output_path=args.output_path, strict=bool(args.strict))
        payload = [{"stage": "dynamics_manifest_from_observed_transitions", "output_path": result.manifest_path, "diagnostics": result.diagnostics}]
        ...
        if args.stage in {"renderer", "temporal_refinement", "dynamics_transition"}:
